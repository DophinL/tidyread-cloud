...
如果是自动生成，无需做任何优化。若手动生成，等太久的话会消耗我们的耐心，即使它是异步的并不会阻塞我们做其他事情。

若是手动生成，你可以调整以下参数：
![improve_digest_speed](https://tidyread.s3.us-west-2.amazonaws.com/improve_digest_speed.png)

`Max Items Per RSS Feed` 代表每个阅读源每次抓取多少篇文章，这个值越小，要摘要的文章越少，速度越快。

`Max Summarize API Concurrency` 代表同时调用多少个摘要 API，这个值越大，速度越快，但是取受制于 AI Provider 的限制，比如 `Moonshot AI` 目前只允许并发为 3。
